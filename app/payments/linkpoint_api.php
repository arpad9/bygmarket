...
$host = $processor_data['processor_params']['secure_host'];
$port = $processor_data['processor_params']['secure_port'];
$sert = Registry::get('config.dir.payments') . 'certificates/' . $processor_data['processor_params']['cert_path'];
$o_prefix = $processor_data['processor_params']['order_prefix'];

$addrnum = preg_replace("/[^\d]/", "", $order_info['b_address']);
$test_mode = $processor_data['processor_params']['test'];

$post_data = array();
$post_data[] = '<order><orderoptions>';
$post_data[] = '<ordertype>' . $processor_data['processor_params']['transaction_type'] . '</ordertype>';
...
$post_data[] = '<cvmindicator>' . $processor_data['processor_params']['cvm_check'] . '</cvmindicator></creditcard>';

$post_data[] = '<merchantinfo><configfile>' . $processor_data['processor_params']['store'] . '</configfile>';
$post_data[] = '<keyfile>' . $processor_data['processor_params']['cert_path'] . '</keyfile>';
$post_data[] = '<host>' . $processor_data['processor_params']['secure_host'] . '</host><port>' . $processor_data['processor_params']['secure_port'] . '</port></merchantinfo>';
...
$__response = Http::post("https://$host:$port/LSGSXML" , implode("\n", $post_data), array(
    'ssl_cert' => $sert,
    'ssl_key' => $sert
));
