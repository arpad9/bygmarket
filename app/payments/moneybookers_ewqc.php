...
//		$pp_response['mb_currency'] = $mb_currency;
    ...
    $post_data = array(
        ...
        'return_url' => fn_url("payment_notification.return?payment=moneybookers_ewqc&order_id=$order_id$suffix", AREA, 'current'),
        'return_url_text' => '',
        'cancel_url' => fn_url("payment_notification.cancel?payment=moneybookers_ewqc&order_id=$order_id$suffix", AREA, 'current'),

        'status_url' => fn_url("payment_notification.status?payment=moneybookers_ewqc&order_id=$order_id$suffix", AREA, 'current'),
    ...
    if (!$post_data['amount']) {
        if (!empty($suffix)) {
            ...
            $url = fn_url("payment_notification.unsupported_currency?payment=moneybookers_ewqc&order_id=$order_id", AREA, 'current');
            echo <<<EOT
