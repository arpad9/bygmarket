...
if (defined('PAYMENT_NOTIFICATION')) {
    if ($mode == 'notify') {
        ...
        $_signature = md5($_REQUEST['bpe_trx'] . $_REQUEST['bpe_timestamp'] . $processor_data["processor_params"]["merchant_id"] . $_REQUEST['order_id'] . $_order_id .
        $processor_data["processor_params"]["currency"] . $_order_total . $_REQUEST['bpe_result'] . $processor_data["processor_params"]["test"] . $processor_data["processor_params"]["merchant_key"]);
...
$_order_total = $order_info['total'] * 100;
$return_url = fn_url("payment_notification.notify?payment=ideal_xml&order_id=$order_id", AREA, 'current');

$signature = md5($processor_data['processor_params']['merchant_id'] . $_order_id . $_order_total . $processor_data['processor_params']['currency'] . $processor_data['processor_params']['test'] . $processor_data['processor_params']['merchant_key']);

$post_url = ($processor_data['processor_params']['payment_type'] == 'cc') ? "https://payment.buckaroo.nl/sslplus/request_for_authorization.asp" : "https://payment.buckaroo.nl/gateway/ideal_payment.asp";

echo <<<EOT
<form method="post" action="{$post_url}" name="process">
    <input type="hidden" name="BPE_Merchant" value="{$processor_data['processor_params']['merchant_id']}">
    <input type="hidden" name="BPE_Signature2" value="{$signature}">
    <input type="hidden" name="BPE_Amount" value="{$_order_total}">
    <input type="hidden" name="BPE_Description" value="{$processor_data['processor_params']['description']}">
    <input type="hidden" name="BPE_Currency" value="{$processor_data['processor_params']['currency']}">
    <input type="hidden" name="BPE_Invoice" value="{$order_id}">
    <input type="hidden" name="BPE_Language" value="{$processor_data['processor_params']['language']}">
    <input type="hidden" name="BPE_Mode" value="{$processor_data['processor_params']['test']}">
