...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    if (AREA == 'A') {
        ...
        if (empty($_REQUEST['payment_id']) && $mb_exit_needed) {
            fn_set_notification('W', __('warning'), __('text_moneybookers_payment_is_not_saved'));
            exit;
        }
        ...
        if ($mode == 'validate_email') {
            ...
            if (!empty($_REQUEST['email']) && !empty($_REQUEST['payment_id'])) {
                ...
                $result = Http::get("https://www.moneybookers.com/app/email_check.pl", $get_data);

                $result_array = explode(',', $result);

                if ($result_array[0] == 'OK') {
                    $processor_params['customer_id'] = $result_array[1];
                    fn_set_notification('N', __('notice'), __('text_moneybookers_email_is_registered'));
                } else {
                    fn_set_notification('E', __('error'), __('text_moneybookers_email_is_not_registered'));
                }
        ...
        if ($mode == 'activate') {
            ...
            if (!empty($_REQUEST['payment_id']) && !empty($_REQUEST['email']) && !empty($_REQUEST['cust_id']) && !empty($_REQUEST['platform']) && !empty($_REQUEST['merchant_firstname']) && !empty($_REQUEST['merchant_lastname'])) {
                ...
                Mailer::sendMail(array(
                    ...
                    'tpl' => 'payments/cc_processors/activate_moneybookers.tpl'
                ), 'A', Registry::get('settings.Appearance.backend_default_language'));

                fn_set_notification('W', __('important'), __('text_moneybookers_activate_quick_checkout_short_explanation_1', array(
                    '[date]' => date('m.d.Y')
                )));

            } else {
                fn_set_notification('E', __('error'), __('text_moneybookers_empty_input_data') );
            }
        }
        ...
        if ($mode == 'validate_secret_word') {
            ...
            if (!empty($_REQUEST['email']) && !empty($_REQUEST['payment_id']) && !empty($_REQUEST['cust_id']) && !empty($_REQUEST['secret'])) {
                ...
                $result = Http::get("https://www.moneybookers.com/app/secret_word_check.pl", $get_data);

                $result_array = explode(',', $result);

                if ($result_array[0] == 'OK') {
                    fn_set_notification('N', __('notice'), __('text_moneybookers_secret_word_is_correct'));
                } else {
                    fn_set_notification('E', __('error'), __('text_moneybookers_secret_word_is_incorrect', array(
                        '[date]' => date('m.d.Y')
                    )));
                }
...
//		$pp_response['mb_currency'] = $mb_currency;
    ...
    $post_data = array(
        ...
        'transaction_id' => $processor_data['processor_params']['order_prefix'] . (($order_info['repaid']) ? ($order_id . '_' . $order_info['repaid']) : $order_id),
        'return_url' => fn_url("payment_notification.return?payment=moneybookers&order_id=$order_id", AREA, 'current'),
        'return_url_text' => '',
        'cancel_url' => fn_url("payment_notification.cancel?payment=moneybookers&order_id=$order_id", AREA, 'current'),

        'status_url' => fn_url("payment_notification.status?payment=moneybookers&order_id=$order_id", AREA, 'current'),

        'language' => $processor_data['processor_params']['language'],

        // iframe_target
        'return_url_target' => $processor_data['processor_params']['return_url_target'],
        'cancel_url_target' => $processor_data['processor_params']['cancel_url_target'],
    ...
    if (!$post_data['amount']) {
        ...
        $url = fn_url("payment_notification.unsupported_currency?payment=moneybookers&order_id=$order_id", AREA, 'current');
        echo <<<EOT
...
EOT;
    ...
    if (!empty($processor_data['processor_params']['payment_methods'])) {
        $post_data['payment_methods'] .= ',' . $processor_data['processor_params']['payment_methods'];
    }
    // /split_gateway
    ...
    $post_data['platform'] = $processor_data['processor_params']['platform'];
...
$msg = __('text_cc_processor_connection', array(
    '[processor]' => 'Moneybookers.com server'
));
echo <<<EOT
    </form>
