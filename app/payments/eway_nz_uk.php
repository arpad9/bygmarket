...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    $return_url = fn_url("payment_notification.notify?payment=eway_nz_uk&order_id=$order_id", AREA, 'current');
    $MerchantInvoice = $order_prefix . (($order_info['repaid']) ? ($order_id . '_' . $order_info['repaid']) : $order_id);
    $currency = ($processor_data['processor_params']['gateway'] == 'NZ') ? 'NZD' : 'GBP';
