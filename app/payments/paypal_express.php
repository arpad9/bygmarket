...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    if ($mode == 'cancel') {
        ...
        $pp_username =  $processor_data['processor_params']['username'];
        $pp_password = $processor_data['processor_params']['password'];
        ...
        if (!empty($processor_data['processor_params']['authentication_method']) && $processor_data['processor_params']['authentication_method'] == 'signature') {
            $signature = '<Signature>' . $processor_data['processor_params']['signature'] . '</Signature>';
            $url_prefix = '-3t';
        } else {
            $cert_file = Registry::get('config.dir.payments') . 'certificates/' . $processor_data['processor_params']['certificate'];
        }

        $pp_order_id = $processor_data['processor_params']['order_prefix'] . (($order_info['repaid']) ? ($_REQUEST['order_id'] . '_' . $order_info['repaid']) : $_REQUEST['order_id']);
...
if (!empty($_payment_id) && (!empty($_SESSION['cart']['products']) || !empty($_SESSION['cart']['gift_certificates'])) && Registry::get('runtime.mode') == 'cart') {
    ...
    if ((!empty($_payment_id) && (!empty($_SESSION['cart']['products']) || !empty($_SESSION['cart']['gift_certificates'])) && $_SERVER['REQUEST_METHOD'] == "POST" && !empty($_REQUEST['mode']) && $_REQUEST['mode'] == "express") || ($pp_method == 'mf')) {
        ...
        if ($pp_method == 'sf') {
            $return_url = htmlspecialchars(fn_payment_url('current', "paypal_express.php?mode=express_return&_payment_id=$_payment_id"));
            $cancel_url = fn_url("checkout.cart", 'C', 'current');

        } else {
            $return_url = htmlspecialchars(fn_url("payment_notification.notify?payment=paypal_express&order_id=$order_id", 'C', 'current'));
            $cancel_url = htmlspecialchars(fn_url("payment_notification.cancel?payment=paypal_express&order_id=$order_id", 'C', 'current'));
        }
        ...
        $_address = '';
        if ($pp_method == 'mf' && !empty($processor_data['processor_params']['send_adress']) && $processor_data['processor_params']['send_adress'] == 'Y') {
$_address = <<<EOT
...
EOT;
        ...
        if ($result['success'] && !empty($result['Token'])) {
            $submit_url = $payment_url . '/webscr';
            $post_data = array(
                'cmd' => '_express-checkout',
                'token' => $result['Token'],
            );
            if ($pp_method == 'mf') {
                $post_data['useraction'] = 'commit';
            }
            fn_create_payment_form($submit_url, $post_data, 'Paypal');
        }
        ...
        if ($pp_method == 'mf') {
            ...
            fn_set_notification('E', __('error'), isset($result['error']['LongMessage'])? $result['error']['LongMessage'] : $result['error']['ShortMessage'] );
...
EOT;
        ...
        fn_redirect(fn_url("checkout.checkout?payment_id=" . $_payment_id, 'C'));

    } elseif (!empty($mode) && $mode == 'place_order') {
        $pp_order_id = $processor_data['processor_params']['order_prefix'] . (($order_info['repaid']) ? ($order_id . '_' . $order_info['repaid']) : $order_id);
...
function fn_paypal_request($request, $post_url, $cert_file)
{
    ...
    $result['response'] = Http::post($post_url, $request, array(
        'headers' => array(
    ...
    if (empty($result['response'])) {
        return array(
            'success' => false,
            'error' => array ('ShortMessage' => Http::getError(), 'LongMessage' => Http::getError(), 'ErrorCode' => 0)
        );
    }
