...
class Styles
{
    ...
    public function setStyle($layout_id, $style_id)
    {
        ...
        if (
            (fn_allowed_for('ULTIMATE') && !empty($this->company_id) || fn_allowed_for('MULTIVENDOR')) 
            && !empty($layout_id)
            && !empty($style_id)
        ...
        foreach ($this->schema['fonts']['fields'] as $field => $data) {
            ...
            if (empty($this->schema['fonts']['families'][$font_name])) {
                ...
                if (empty($content[$font_name])) {
                    $font_data = $this->getGoogleFontData($font_name);

                    // Set user agent manually to get IE-specific code
                    $css = Http::get('http://fonts.googleapis.com/css?family=' . $font_name . (!empty($font_data['weight']) ? ':' . $font_data['weight'] : ''), array(), array(
     ...
     */
    private function getCustomFonts($less)
    {
        $families = array();
        if (preg_match("#/\*{$this->gfonts_tag}\*/(.*?)/\*/{$this->gfonts_tag}\*/#s", $less, $matches)) {
            if (preg_match_all('/font-family: \'([\w ]+)\';/', $matches[1], $fonts)) {
                $families = $fonts[1];
            }
        }
