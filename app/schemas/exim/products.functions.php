            ...
            foreach ($categories as $cat) {
                ...
                if (fn_allowed_for('ULTIMATE')) {
                    if (!empty($paths_store[$key_path]) && !Registry::get('runtime.company_id')) {
                        $company_id = fn_get_company_id_by_name($paths_store[$key_path]);
...
function fn_exim_get_product_features($product_id, $features_delimiter, $lang_code = CART_LANGUAGE)
{
    ...
    if (!isset($features[$lang_code])) {
        list($features[$lang_code]) = fn_get_product_features($_params, 0, $lang_code);
    }
    ...
    if (!empty($features[$lang_code])) {
        foreach ($features[$lang_code] as $f) {
            $parent = '';
 ...
 */
function fn_exim_set_product_features($product_id, $data, $features_delimiter)
{
    //for compatibility with the old format
    $data = preg_replace('{\{\d*\}}', '', $data);

    if (!fn_is_empty($data)) {
        $error_feature_names = array();

        $multilang_data = fn_exim_parse_data($data, $features_delimiter);
        ...
        unset($langs[$main_lang]);

        foreach ($multilang_data as $feature) {

            // import features
            if (!empty($feature['group_name'])) {
                $group_id = fn_exim_check_feature_group($feature['group_name'], $company_id);
            }

            $condition = db_quote("WHERE description = ?s AND lang_code = ?s AND feature_type = ?s", $feature['name'][$main_lang], $main_lang, $feature['type']);
            if (empty($group_id)) {
                $group_id = 0;
            }
            ...
            if (empty($feature_id)) {
                $feature_data = array(
                    'description' => $feature['name'][$main_lang],
                ...
                foreach ($langs as $lang_code => $lang_data) {
                    $feature_desc = (!empty($feature['name'][$lang_code])) ? $feature['name'][$lang_code] : $feature['name'][$main_lang];
                    ...
                    fn_update_product_feature($feature_data, $feature_id, $lang_code);
                }
            }
            ...
            fn_exim_product_feature_variants($feature, $feature_id, $variants, $multi_lang);
        }

        foreach ($multi_lang as $lang_code) {
            fn_update_product_features_value($product_id, $variants[$lang_code], array(), $lang_code);
        }
    }
...
function fn_exim_product_feature_variants($feature, $feature_id, &$variants, $multi_lang)
{
    $feature_type = $feature['type'];
    ...
    if (strpos('MSNE', $feature_type) !== false) { // variant IDs
        ...
        $existent_variants = db_get_hash_single_array(
            ...
            array('variant_id', 'variant'), $feature_id, $vars_main, $main_lang
        );

        foreach ($feature['variants'] as $variant_data) {
            if (!in_array($variant_data[$main_lang], $existent_variants)) {
                $variant_id = fn_add_feature_variant($feature_id, array('variant' => $variant_data[$main_lang]));
                $existent_variants[$variant_id] = $variant_data[$main_lang];
            }
        }
        ...
        if ($feature_type == 'M') {
            ...
            foreach ($feature['variants'] as $variant_data) {
                foreach ($variant_data as $lang_code => $variant_value) {
                    if (in_array($variant_value, $existent_variants)) {
                        $variant_id = array_search($variant_value, $existent_variants);
                        $variants[$lang_code][$feature_id][$variant_id] = $variant_id;
                    }
                }
            }
        } else {
            ...
            foreach ($variant_data as $lang_code => $variant_value) {
                if (in_array($variant_value, $existent_variants)) {
                    $variant_id = array_search($variant_value, $existent_variants);
                    $variants[$lang_code][$feature_id] = $variant_id;
                }
            }
        }
        ...
        $variant_data = reset($feature['variants']);
        foreach ($variant_data as $lang_code => $variant_value) {
            $variants[$lang_code][$feature_id] = $variant_value;
        }
    }
 ...
 */
function fn_exim_check_feature_group($group, $company_id)
{
    if (!is_array($group)) {
        return false;
    }

    $multi_lang = array_keys($group);
    $main_lang = reset($multi_lang);

    $group_id = db_get_field("SELECT feature_id FROM ?:product_features_descriptions WHERE description = ?s AND lang_code = ?s LIMIT 1", $group[$main_lang], $main_lang);

    if (empty($group_id)) {

        foreach (fn_get_translation_languages() as $lang_code => $lang_data) {
            $group_desc = !empty($group[$lang_code]) ? $group[$lang_code] : $group[$main_lang];
            $group_data = array(
                'feature_id' => $group_id,
            ...
            $group_id = fn_update_product_feature($group_data, $group_id, $lang_code);
        }
...
function fn_exim_set_product_options($product_id, $data)
{
    //for compatibility with the old format
    $data = preg_replace('{\{\d*\}}', '', $data);
    ...
    if (!fn_is_empty($data)) {
        ...
        $data_multilang = fn_exim_parse_data($data);

        $multi_lang = array_keys($data);
        $main_lang = array_shift($multi_lang);

        $updated_ids = array(); // store updated ids, delete other (if exist)

        foreach ($data_multilang as $option_key => $option) {

            $global_option = (isset($option['global'])) ? $option['global'] : false;

            if (!empty($option['group_name'][$main_lang])) {
                $company_id = fn_get_company_id_by_name($option['group_name'][$main_lang]);
            }

            $option_id = db_get_field("SELECT o.option_id FROM ?:product_options_descriptions as d INNER JOIN ?:product_options as o ON o.option_id = d.option_id AND o.product_id = ?i WHERE d.option_name = ?s AND d.lang_code = ?s LIMIT 1", ($global_option) ? 0 : $product_id, $option['name'][$main_lang], $main_lang);

            $variant_ids = array();
            foreach ($option['variants'] as $variant_pos => $variant) {
                $variant_ids[$variant_pos] = db_get_field("SELECT d.variant_id FROM ?:product_option_variants_descriptions as d INNER JOIN ?:product_option_variants as o ON o.variant_id = d.variant_id AND o.option_id = ?i WHERE d.variant_name = ?s AND d.lang_code = ?s LIMIT 1", $option_id, $variant[$main_lang], $main_lang);
            }

            $option_data = fn_exim_build_option_data($option, $option_id, $variant_ids, $main_lang);
            ...
            if (empty($option_id)) {
                ...
                $updated_id = fn_update_product_option($option_data, 0, $main_lang);

                // Option is exist, update it
            } else {
                $updated_id = fn_update_product_option($option_data, $option_id, $main_lang);
            }
            ...
            $variant_ids = array();
            foreach ($option['variants'] as $variant_pos => $variant) {
                $variant_ids[$variant_pos] = db_get_field("SELECT d.variant_id FROM ?:product_option_variants_descriptions as d INNER JOIN ?:product_option_variants as o ON o.variant_id = d.variant_id AND o.option_id = ?i WHERE d.variant_name = ?s AND d.lang_code = ?s LIMIT 1", $updated_id, $variant[$main_lang], $main_lang);
            }

            // update multilang options
            foreach ($multi_lang as $lang_code) {
                $option_data = fn_exim_build_option_data($option, $option_id, $variant_ids, $lang_code);
                fn_update_product_option($option_data, $updated_id, $lang_code);
            }
...
function fn_exim_parse_data($data, $variants_delimiter = ',')
{
    ...
    foreach ($data as $lang_code => $options) {
        ...
        $options = explode($set_delimiter, $options);
        ...
        foreach ($options as $option) {
            ...
            if (is_array($pair)) {
                ...
                $data_multilang[$o_position]['type'] = substr($pair[1], 0, 1);
                if (substr($pair[1], 1, 1) == 'G') {
                    $data_multilang[$o_position]['global'] = true;
                }

                $data_multilang[$o_position]['name'][$lang_code] = $pair[0];

                if (!empty($group[1])) {
                    $data_multilang[$o_position]['group_name'][$lang_code] = $group[1];
                }
                ...
                foreach ($variants as $variant) {
                    $position += 10;
                    $data_multilang[$o_position]['variants'][$position][$lang_code] = trim($variant);
                }
            }
        }
    }

    return $data_multilang;
}
...
function fn_exim_build_option_data($option, $option_id, $variant_ids, $lang_code)
{
    ...
    if ($option['type'] == 'C') {
        ...
        foreach ($option['variants'] as $variant_pos => $variant) {
            $variants[$variant_pos] = array(
                'variant_name' => $variant[$lang_code],
    ...
    $option_data = array(
        'option_name' => $option['name'][$lang_code],
