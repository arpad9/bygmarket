...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'updateregistry') {
        ...
        echo '</pre>';
        echo ">>".$_REQUEST['registry_data']['wr_id'];*/
        //die('here... yupppp');
        
        if (!empty($_REQUEST['registry_data'])) {
            list($registry_id) = fn_update_registry($_REQUEST['registry_data'], $_REQUEST['registry_data']['wr_id']);
        }
		//echo ">>".$registry_id;
		//die('here... yupppp');
		...
		Registry::get('view')->assign('wr_id', $_REQUEST['registry_data']['wr_id']);
		fn_set_notification('N', __('notice'), "Your Wedding Registry updated successfully");
        $suffix = ".addreg?wr_id=$registry_id";
    ...
    if ($mode == 'add_products') {
		...
		*/
        ...
        if (!empty($_REQUEST['product_data'])) {
            fn_update_wedding_registry_products($_REQUEST['registry_type_id'], $_REQUEST['product_data'], true);
        }
    ...
    if ($mode == 'update') {
        ...
        if (!empty($_REQUEST['event_products'])) {
            ...
            unset($_REQUEST['event_products']['custom_files']);
            fn_update_wedding_registry_products($_REQUEST['registry_type_id'], $_REQUEST['event_products']);
        }
        $suffix = ".update?registry_type_id=$registry_type_id";
        
    }

    return array(CONTROLLER_STATUS_OK, "wregistry$suffix");
}
...
if ($mode == 'update') {
	...
	if(!empty($auth['user_id'])){
		$registry_data = db_get_row("SELECT * FROM ?:wedding_registry WHERE wr_id = ?i AND user_id = ?i", $_REQUEST['registry_type_id'], $auth['user_id']);
    ...
    $event_data = db_get_row("SELECT * FROM ?:wedding_registry WHERE wr_id = ?i", $_REQUEST['registry_type_id']);
    
    list($event_data['products'], $search) = fn_get_wedding_registry_products($param, Registry::get('settings.Appearance.products_per_page'));
	//die('B4 function');
    ...
    if (AREA != 'A') {
        fn_add_breadcrumb("Wedding Registry", "wregistry.home");
        fn_add_breadcrumb("Manage Wedding Registry List", "wregistry.update&registry_type_id=".$_REQUEST['registry_type_id']);
    }
...
if ($mode == 'view'){
	...
	if(!empty($auth['user_id'])){
		$registry_data = db_get_row("SELECT * FROM ?:wedding_registry WHERE wr_id = ?i AND type = ?s", $_REQUEST['registry_type_id'], 'P');
...
if ($mode == 'home') {
    ...
    $data = db_get_row("SELECT wr_id, user_id FROM ?:wedding_registry WHERE user_id = ?s", $auth['user_id']);
	...
	Registry::get('view')->assign('wr_id', $data['wr_id']);
...
if ($mode == 'addreg') {
	...
	if (!empty($_REQUEST['wr_id'])) {
		$registry_data = db_get_row("SELECT * FROM ?:wedding_registry WHERE wr_id = ?i AND user_id = ?i", $_REQUEST['wr_id'], $auth['user_id']);
		
		// Displays no page - if user tries to view other users profile
		if($_REQUEST['wr_id'] != $registry_data['wr_id']){
			return array(CONTROLLER_STATUS_NO_PAGE);
		}
    ...
    Registry::get('view')->assign('registry_type_id', $registry_data['wr_id']);
	
	//return array(CONTROLLER_STATUS_OK, "wregistry.addreg".$suffix);
	if (AREA != 'A' && !empty($data)) {
        fn_add_breadcrumb("Wedding Registry", "wregistry.home");
        fn_add_breadcrumb("Wedding Registry List", "wregistry.update&registry_type_id=".$registry_data['wr_id']);
        fn_add_breadcrumb("Manage Wedding Registry", "wregistry.addreg&wr_id=".$registry_data['wr_id']);
    }else{
		fn_add_breadcrumb("Wedding Registry", "wregistry.home");
        fn_add_breadcrumb("Add Wedding Registry", "wregistry.addreg");
	}
} 
...
if ($mode == 'delete_product' && !empty($_REQUEST['item_id']) && !empty($_REQUEST['registry_type_id'])) {
    ...
    if (!empty($auth['user_id'])) {
        ...
        if (!empty($_REQUEST['registry_type_id'])) {
            ...
            db_query("DELETE FROM ?:registry_products WHERE item_id = ?i AND registry_type_id = ?i AND registry_type = ?s", $_REQUEST['item_id'], $_REQUEST['registry_type_id'], 'wr');
            fn_set_notification('N', __('notice'), 'Product Has Been Deleted');
        }
        
    }

    return array(CONTROLLER_STATUS_REDIRECT, "wregistry.update".$suffix);
}

if ($mode == 'delete_registry' && !empty($_REQUEST['wr_id'])) {
    ...
    if (!empty($auth['user_id'])) {
        ...
        if (!empty($_REQUEST['wr_id'])) {
			...
			// Get current users wedding registry ID
			$current_wr_id = db_get_field("SELECT wr_id FROM ?:wedding_registry WHERE user_id = ?i", $auth['user_id']);
			
			// Displays no page - if user tries to view other users profile
			if($_REQUEST['wr_id'] != $current_wr_id){
				return array(CONTROLLER_STATUS_NO_PAGE);
			}
			
            db_query("DELETE FROM ?:wedding_registry WHERE wr_id = ?i AND user_id = ?i", $_REQUEST['wr_id'], $auth['user_id']);
            db_query("DELETE FROM ?:registry_products WHERE registry_type_id = ?i AND user_id = ?i AND registry_type = ?s", $_REQUEST['wr_id'], $auth['user_id'], 'wr');
            fn_set_notification('N', __('notice'), 'Your Registry Has Been Deleted');
        }
        
    }
	
    return array(CONTROLLER_STATUS_REDIRECT, "wregistry.home");
}


//////////////////////////////////////////

function fn_get_wedding_registry_products($params, $items_per_page = 0, $lang_code = CART_LANGUAGE)
{
...
function fn_update_registry($event_data, $event_id = 0)
{
    ...
    if (empty($event_id)) {
		//die($event_id."--if");
        $event_id = db_query("INSERT INTO ?:wedding_registry ?e", $_data);
    } else {
		//die($event_id."--else");
        unset($_data['user_id']);
        db_query("UPDATE ?:wedding_registry SET ?u WHERE wr_id = ?i", $_data, $event_id);
    }

    return array($event_id);
}

function fn_update_wedding_registry_products($registry_type_id, $event_products, $is_add = false)
{
    foreach ($event_products as $item_id => $data) {
        ...
        $data['registry_type'] = 'wr';
        $data['registry_type_id'] = $registry_type_id;
		$data['user_id'] = $user_id = $_SESSION['auth']['user_id'];
		$data['lastadded'] = TIME;
		
        if ($is_add || $data['item_id'] != $item_id) {
			$existent_amount = db_get_field("SELECT amount FROM ?:registry_products WHERE item_id = ?i AND registry_type_id = ?i AND registry_type = ?s", $data['item_id'], $registry_type_id, 'wr');
            //die('if');
            $data['product_id'] = $product_id;
            if (!$is_add) {
                db_query("DELETE FROM ?:registry_products WHERE item_id = ?i AND registry_type_id = ?i AND registry_type = ?s", $data['item_id'], $registry_type_id, 'wr');
            }
            ...
            if (!empty($data['amount'])) {
				if($data['amount'] != $existent_amount){
					db_query("UPDATE ?:registry_products SET ?u WHERE item_id = ?i AND registry_type_id = ?i AND registry_type = ?s", $data, $item_id, $registry_type_id, 'wr');
				}
