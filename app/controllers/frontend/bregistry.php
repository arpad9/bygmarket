...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'updateregistry') {
        ...
        if (!empty($_REQUEST['registry_data'])) {
            list($registry_id) = fn_update_registry($_REQUEST['registry_data'], $_REQUEST['registry_data']['br_id']);
        }
		/*echo ">>".$registry_id;
		die('here... yupppp');
        */
		...
		Registry::get('view')->assign('br_id', $registry_id);
		fn_set_notification('N', __('notice'), "Your Birthday Registry updated successfully");
        //$suffix = ".addreg?br_id=$registry_id";
        $suffix = ".manage";
    ...
    if ($mode == 'add_products') {
		...
		*/
        ...
        if (!empty($_REQUEST['product_data'])) {
            fn_update_birthday_registry_products($_REQUEST['registry_type_id'], $_REQUEST['product_data'], true);
        }
    ...
    if ($mode == 'update') {
        ...
        if (!empty($_REQUEST['event_products'])) {
            ...
            unset($_REQUEST['event_products']['custom_files']);
            fn_update_birthday_registry_products($_REQUEST['registry_type_id'], $_REQUEST['event_products']);
        }
        $suffix = ".update?registry_type_id=$registry_type_id";
        
    }

    return array(CONTROLLER_STATUS_OK, "bregistry$suffix");
}
...
if ($mode == 'update') {
	...
	if(!empty($auth['user_id'])){
		$registry_data = db_get_row("SELECT * FROM ?:birthday_registry WHERE br_id = ?i AND user_id = ?i", $_REQUEST['registry_type_id'], $auth['user_id']);
    ...
    $event_data = db_get_row("SELECT * FROM ?:birthday_registry WHERE br_id = ?i", $_REQUEST['registry_type_id']);
    
    list($event_data['products'], $search) = fn_get_birthday_registry_products($param, Registry::get('settings.Appearance.products_per_page'));
	//die('B4 function');
    ...
    if (AREA != 'A') {
        fn_add_breadcrumb("Birthday Registry", "bregistry.home");
        fn_add_breadcrumb("Manage Birthday Registry List", "bregistry.manage");
        fn_add_breadcrumb("Manage Birthday Registry Product List", "bregistry.update&registry_type_id=".$_REQUEST['registry_type_id']);
    }
...
if ($mode == 'view'){
	...
	if(!empty($auth['user_id'])){
		$registry_data = db_get_row("SELECT * FROM ?:birthday_registry WHERE br_id = ?i AND type = ?s", $_REQUEST['registry_type_id'], 'P');
    ...
    $interests = array('1'=>'Arts & Crafts',
					   '2'=>'Cars, Trains & Planes',
					   '3'=>'Building',
					   '4'=>'Learning & Science',
					   '5'=>'Animals',
					   '6'=>'Princesses',
	...
	Registry::get('view')->assign('interests', $interests);
...
if ($mode == 'home') {
    ...
    $data = db_get_row("SELECT br_id, user_id FROM ?:birthday_registry WHERE user_id = ?s", $auth['user_id']);
	if (!empty($data)) {
		$registry_data = $data;
	}
	
	Registry::get('view')->assign('br_id', $data['br_id']);
}
...
if ($mode == 'addreg') {
	...
	$data = db_get_row("SELECT * FROM ?:wedding_registry WHERE user_id = ?s", $auth['user_id']);
	if (!empty($data)) {
		$registry_data = $data;
	...
	*/
	...
	if (!empty($_REQUEST['br_id'])) {
		$registry_data = db_get_row("SELECT * FROM ?:birthday_registry WHERE br_id = ?i AND user_id = ?i", $_REQUEST['br_id'], $auth['user_id']);
		
		// Displays no page - if user tries to view other users profile
		if($_REQUEST['br_id'] != $registry_data['br_id']){
			return array(CONTROLLER_STATUS_NO_PAGE);
		}
		
		$registry_data['interests'] = explode(',', $registry_data['interests']);
    ...
    Registry::get('view')->assign('registry_type_id', $registry_data['br_id']);
	
	//return array(CONTROLLER_STATUS_OK, "wregistry.addreg".$suffix);
	
	if (AREA != 'A' && !empty($_REQUEST['br_id'])) {
        fn_add_breadcrumb("Birthday Registry", "bregistry.home");
        fn_add_breadcrumb("Manage Birthday Registry List", "bregistry.manage");
        fn_add_breadcrumb("Manage Birthday Registry", "bregistry.addreg&br_id=".$registry_data['br_id']);
    }else{
		fn_add_breadcrumb("Birthday Registry", "bregistry.home");
		fn_add_breadcrumb("Manage Birthday Registry List", "bregistry.manage");
        fn_add_breadcrumb("Add Birthday Registry", "bregistry.addreg");
	}
...
if ($mode == 'manage') {
	...
	$registry_data = db_get_array("SELECT * FROM ?:birthday_registry WHERE user_id = ?i", $auth['user_id']);
...
if ($mode == 'delete_product' && !empty($_REQUEST['item_id']) && !empty($_REQUEST['registry_type_id'])) {
    ...
    if (!empty($auth['user_id'])) {
        ...
        if (!empty($_REQUEST['registry_type_id'])) {
            ...
            db_query("DELETE FROM ?:registry_products WHERE item_id = ?i AND registry_type_id = ?i AND registry_type = ?s", $_REQUEST['item_id'], $_REQUEST['registry_type_id'], 'br');
            fn_set_notification('N', __('notice'), 'Product Has Been Deleted');
        }
        
    }

    return array(CONTROLLER_STATUS_REDIRECT, "bregistry.update".$suffix);
}

if ($mode == 'delete_registry' && !empty($_REQUEST['br_id'])) {
    ...
    if (!empty($auth['user_id'])) {
        ...
        if (!empty($_REQUEST['br_id'])) {
			...
			// Get current users birthday registry ID's
			$current_br_id = db_get_field("SELECT br_id FROM ?:birthday_registry WHERE br_id = ?i AND user_id = ?i", $_REQUEST['br_id'], $auth['user_id']);
			
			// Displays no page - if user tries to view other users profile
			if($_REQUEST['br_id'] != $current_br_id){
				return array(CONTROLLER_STATUS_NO_PAGE);
			}
			
            db_query("DELETE FROM ?:birthday_registry WHERE br_id = ?i AND user_id = ?i", $_REQUEST['br_id'], $auth['user_id']);
            db_query("DELETE FROM ?:registry_products WHERE registry_type_id = ?i AND user_id = ?i AND registry_type = ?s", $_REQUEST['br_id'], $auth['user_id'], 'br');
            fn_set_notification('N', __('notice'), 'Your Registry Has Been Deleted');
        }
        
    }
	
    return array(CONTROLLER_STATUS_REDIRECT, "bregistry.manage");
}


//////////////////////////////////////////

function fn_get_birthday_registry_products($params, $items_per_page = 0, $lang_code = CART_LANGUAGE)
{
...
{
    $event_data['interests'] = implode(',',$event_data['interests']);;
    ...
    if (empty($event_id)) {
		//die($event_id."--if");
        $event_id = db_query("INSERT INTO ?:birthday_registry ?e", $_data);
    } else {
		//die($event_id."--else");
        unset($_data['user_id']);
        db_query("UPDATE ?:birthday_registry SET ?u WHERE br_id = ?i", $_data, $event_id);
    }

    return array($event_id);
}

function fn_update_birthday_registry_products($registry_type_id, $event_products, $is_add = false)
{
    foreach ($event_products as $item_id => $data) {
        ...
        $data['registry_type'] = 'br';
        $data['registry_type_id'] = $registry_type_id;
		$data['user_id'] = $user_id = $_SESSION['auth']['user_id'];
		$data['lastadded'] = TIME;
		
        if ($is_add || $data['item_id'] != $item_id) {
			$existent_amount = db_get_field("SELECT amount FROM ?:registry_products WHERE item_id = ?i AND registry_type_id = ?i AND registry_type = ?s", $data['item_id'], $registry_type_id, 'br');
            //die('if');
            $data['product_id'] = $product_id;
            if (!$is_add) {
                db_query("DELETE FROM ?:registry_products WHERE item_id = ?i AND registry_type_id = ?i AND registry_type = ?s", $data['item_id'], $registry_type_id, 'br');
            }
            ...
            if (!empty($data['amount'])) {
				if($data['amount'] != $existent_amount){
					db_query("UPDATE ?:registry_products SET ?u WHERE item_id = ?i AND registry_type_id = ?i AND registry_type = ?s", $data, $item_id, $registry_type_id, 'br');
				}
