...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'update_layout') {
        ...
        Layout::instance()->update($layout_data, $_REQUEST['layout_id']);
    }
    ...
    if ($mode == 'export_locations') {
        ...
        $content = Exim::instance()->export($location_ids, $_REQUEST);
        ...
        fn_mkdir(dirname(Registry::get('config.dir.root') . '/var/layouts/' . $filename));
        file_put_contents(Registry::get('config.dir.root') . '/var/layouts/' . $filename, $content);
...
if ($mode == 'manage' || $mode == 'manage_in_tab') {
    ...
    if (!empty($_REQUEST['to_screen'])) {
        ...
        readfile(Registry::get('config.dir.root') . '/var/layouts/' . $file);
        exit;

    } else {
        fn_get_file(Registry::get('config.dir.root') . '/var/layouts/' . $file);
    }
