...
Registry::set('settings.store_optimization_dev', 'Y');
    ...
    if (is_file($path)) {
        ...
        if (file_exists($original_file)) {
            if (md5_file($original_file) != md5_file($path)) {
                ...
                if (fn_allowed_for('ULTIMATE')) {
                    if (preg_match('!themes/\d+!S', $relative_file)) {
                        $_relative_file = preg_replace('!^.*/themes/\d+/[\w]+/!S', 'var/themes_repository/basic/', $relative_file);
                        ...
                        if (fn_uc_check_array_value($relative_file, $custom_theme_files) && strpos($relative_file, '/basic/') === false) {
                            $_relative_file = preg_replace('!^.*/themes/\d+/([\w]+)/!S', 'var/themes_repository/${1}/', $relative_file);
                        }
                    }
                }
                if (strpos($relative_file, 'themes/') === 0) {
                    $_relative_file = preg_replace('!themes/[\w]+/!S', 'var/themes_repository/basic/', $relative_file);

                    // replace all themes except basic
                    if (fn_uc_check_array_value($relative_file, $custom_theme_files) && strpos($relative_file, '/basic/') === false) {
                        $_relative_file = preg_replace('!themes/([\w]+)/!S', 'var/themes_repository/${1}/', $relative_file);
                    }
                }
