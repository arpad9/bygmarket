...
function fn_get_seo_company_condition($field, $object_type = '')
{
    ...
    if (fn_allowed_for('ULTIMATE')) {
        $condition = fn_get_company_condition($field, true, Registry::get('runtime.company_id'));

        // Disable companies in for shared objects
        if (!empty($object_type)) {
            $cache_name = 'seo.seo_object_' . $object_type;
            Registry::registerCache($cache_name, array(), Registry::cacheLevel('static'));
...
function fn_seo_get_categories(&$params, &$join, &$condition, &$fields, &$group_by, &$sortings, &$lang_code)
{
    ...
    $join .= db_quote(
        ...
        $lang_code,  fn_get_seo_join_condition('?:categories.company_id')
    );
}
...
function fn_seo_get_page_data(&$page_data, &$lang_code)
{
    ...
    $page_data['seo_name'] = db_get_field(
        ...
        . " AND ?:seo_names.dispatch = '' AND ?:seo_names.lang_code = ?s ?p",
        $page_data['page_id'], fn_get_corrected_seo_lang_code($lang_code), fn_get_seo_join_condition(Registry::get('runtime.company_id'))
    );

    if (empty($page_data['seo_name'])) {
        // try get seo name without company conditions
        $page_data['seo_name'] = db_get_field(
            "SELECT name FROM ?:seo_names WHERE object_id = ?i AND type = 'a' "
            . " AND ?:seo_names.dispatch = '' AND ?:seo_names.lang_code = ?s",
            $page_data['page_id'], fn_get_corrected_seo_lang_code($lang_code)
        );
    }
 ...
 */
function fn_seo_get_route(&$req, &$result, &$area)
{
    $config = Registry::get('config');
    $current_path = Registry::get('config.current_path');

    if (($area == 'C') && (empty($req['dispatch']) || $req['dispatch'] == '_no_page') && !fn_check_requested_url()) {
...
{
    fn_set_hook('seo_update_objects_pre', $object_data, $object_id, $type, $lang_code, $generate_for_company_ids, $seo_objects);
    ...
    if ((isset($object_data['seo_name']) || !empty($generate_for_company_ids)) && !empty($object_id)) {
        ...
        if (fn_allowed_for('ULTIMATE') && isset($object_data['company_id'])) {
        ...
        foreach ($generate_for_company_ids as $_company_id) {
            if (!isset($object_data['seo_name']) || (fn_allowed_for('ULTIMATE') && !(Registry::get('runtime.company_id') && Registry::get('runtime.company_id') == $_company_id))) {
...
function fn_seo_update_product_post(&$product_data, &$product_id, &$lang_code)
{
    ...
    $company_ids = db_get_fields('SELECT company_id FROM ?:categories WHERE category_id IN (?a) AND company_id != ""', explode(',', $product_data['categories']));
...
function fn_seo_get_name($object_type, $object_id = 0, $dispatch = '', $company_id = null, $lang_code = CART_LANGUAGE)
{
    ...
    if (fn_allowed_for('ULTIMATE')) {
        $company_id_condition = fn_get_seo_company_condition('?:seo_names.company_id');
        ...
        if ($company_id !== null) {
            $company_id_condition = db_quote(" AND ?:seo_names.company_id=?i", $company_id);
        }
    }
    ...
    if (empty($name)) {
        ...
        if (Registry::get('seo.lang_code') == $lang_code) {
            $names = Registry::ifGet("seo.names.$object_type", array());
            $name = !empty($names[$_object_id]) ? $names[$_object_id] : false;
        }

        if (empty($name) && !($object_type == 's' && $lang_code == Registry::get('seo.lang_code'))) {
            $where_params = array(
                'object_id' => $object_id,
                'type' => $object_type,
                'dispatch' => $dispatch,
