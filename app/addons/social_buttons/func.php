...
function fn_facebook_prepare_settings($facebook_settings, $params)
{
    ...
    if ($params['object'] == 'products') {
        $data_href = fn_url('products.view?product_id=' . $params['object_id'] . '&lang_code=' . $params['lang_code'] . (!empty($_SESSION['from_url']) ? '#!' . $_SESSION['from_url'] : ''));
    } elseif ($params['object'] == 'pages') {
        $data_href = fn_url('pages.view?page_id=' . $params['object_id'] . '&lang_code=' . $params['lang_code'] . (!empty($_SESSION['from_url']) ? '#!' . $_SESSION['from_url'] : ''));
    }

    $facebook_fields = array(
        'data-lang' => $params['lang_code'],
        'data-layout' => $facebook_settings['facebook_layout'],
        'data-href' => !empty($facebook_settings['facebook_href']) ? $facebook_settings['facebook_href'] : $data_href,
