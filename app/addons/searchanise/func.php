...
function fn_searchanise_dispatch_assign_template()
{
    ...
    if (!fn_allowed_for('ULTIMATE') && fn_check_suppliers_functionality() && fn_se_get_import_status(fn_se_get_company_id(), CART_LANGUAGE) == 'done') {
...
function fn_se_add_action($action, $data = NULL, $company_id = NULL, $lang_code = NULL)
{
    ...
    foreach ($data as $d) {
        foreach ($engines_data as $engine_data) {
            if (($action == 'facet_update' || $action == 'facet_delete' || $action == 'facet_delete_all') && fn_se_get_setting('use_navigation', $engine_data['company_id'], DEFAULT_LANGUAGE) == 'N') {
                continue;
            }
...
function fn_searchanise_tools_change_status(&$params, &$result)
{
    ...
    if ($params['table'] == 'products' && !empty($result)) {
        ...
        if ($params['status'] == 'A') {
            ...
            fn_set_notification('N', __('notice'), __('text_se_import_status_queued_language', array(
...
function fn_searchanise_update_language_post(&$language_data, &$lang_code, &$action)
{
    if (fn_se_signup(NULL, $language_data['lang_code'], false) == true) {
        ...
        fn_set_notification('N', __('notice'), __('text_se_new_engine_language', array(
