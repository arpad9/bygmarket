...
if ($mode == 'async') {
    if (fn_allowed_for('ULTIMATE') && Registry::get('runtime.company_id')) {
        $q = db_get_row("SELECT * FROM ?:se_queue WHERE company_id = ?i ORDER BY queue_id ASC LIMIT 1", Registry::get('runtime.company_id'));
    ...
    while (!empty($q)) {
        ...
        if ($status == true) {
            ...
            if (fn_allowed_for('ULTIMATE') && Registry::get('runtime.company_id')) {
                $q = db_get_row("SELECT * FROM ?:se_queue WHERE company_id = ?i AND queue_id > ?i ORDER BY queue_id ASC LIMIT 1", Registry::get('runtime.company_id'), $q['queue_id']);
            } else {
