...
{if $smarty.const.IS_WINDOWS == false}
...
{capture name="buttons"}
    {capture name="tools_list"}
        ...
        <li class="cm-te-onsite-editing">{btn type="list" text=__("on_site_template_editing") href="site_layout.enable_design_mode?return_url=`$current_url`"|fn_url target="_blank"}</li>
        <li class="divider"></li>
    ...
    <div class="sidebar-row">
        <div class="row">
            ...
            <div class="switch switch-mini" id="rebuild_cache_automatically">
                <input type="checkbox" {if $settings.store_optimization_dev == 'Y'}checked{/if} {if "ULTIMATE"|fn_allowed_for && $runtime.company_id}disabled{/if}/>
            </div>
        </div>
    </div>
    <script type="text/javascript">
        ...
        (function (_, $) {
            $('#rebuild_cache_automatically').on('switch-change', function (e, data) {
                ...
                $.ceAjax('request', '{fn_url("template_editor.enable_rebuild_cache")}', {
                    data: {
                        state: value
                    }
                });
