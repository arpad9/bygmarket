        ...
        {if $gr_feature.feature_type == "ProductFeatures::GROUP"|enum}
            ...
            {if $gr_feature.subfeatures}
                {foreach from=$gr_feature.subfeatures item="subfeature"}
                    ...
                    {else}
                        {include file="common/object_group.tpl" id=$subfeature.feature_id details=$subfeature.feature_description text=$subfeature.description status=$subfeature.status hidden=true href="product_features.update?feature_id=`$subfeature.feature_id`&return_url=$r_url" object_id_name="feature_id" table="product_features" href_delete="product_features.delete?feature_id=`$subfeature.feature_id`&return_url=$r_url" delete_target_id="pagination_contents" header_text="{__("editing_product_feature")}: `$subfeature.description`" update_controller="product_features" no_table=true company_object=$subfeature no_popup=$show_in_popup}
                    {/if}
...
{capture name="adv_buttons"}
    {capture name="tools_list"}
        {capture name="add_new_picker"}
            {include file="views/product_features/update.tpl" feature=[] is_group=true return_url=$config.current_url}
        {/capture}
        ...
        {capture name="add_new_picker_2"}
            {include file="views/product_features/update.tpl" feature=[] return_url=$config.current_url}
        {/capture}
