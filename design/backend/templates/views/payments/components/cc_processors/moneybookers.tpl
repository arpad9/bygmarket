{assign var="register_url" value=__("moneybookers_partner_link")}
{assign var="pay_to_email" value=$processor_params.pay_to_email|escape:url}
<p>{__("text_moneybookers_notice", ["[register_url]" => $register_url])}</p>
<hr>

<div class="control-group">
    <label class="control-label" for="pay_to_email_{$payment_id}">{__("pay_to_email")}:</label>
    <div class="controls">
        <input type="text" name="payment_data[processor_params][pay_to_email]" id="pay_to_email_{$payment_id}" value="{$processor_params.pay_to_email}"  size="60" onchange="Tygh.$('#validate_email_{$payment_id}').prop('href', '{"payment_notification.validate_email?payment=moneybookers&payment_id=`$payment_id`&email="|fn_url:'A':'rel' nofilter}' +  Tygh.$(this).val()); return false;">&nbsp;<a href="{"payment_notification.validate_email?payment=moneybookers&payment_id=`$payment_id`&email=`$processor_params.pay_to_email`"|fn_url}" onclick="Tygh.$.ceAjax('request', Tygh.$(this).prop('href'), {ldelim}method: 'GET', callback: fn_get_validate_email_{$payment_id}{rdelim}); return false;" id="validate_email_{$payment_id}">{__("validate_email")}</a>
    </div>
</div>

<div class="control-group">
    <label class="control-label" for="customer_id_{$payment_id}">{__("moneybookers_customer_id")}:</label>
    <div class="controls">
...
<div class="control-group">
    <label class="control-label" for="platform_{$payment_id}">{__("platform")}:</label>
    <div class="controls">
        <input type="text" name="payment_data[processor_params][platform]" id="platform_{$payment_id}" value="{$processor_params.platform}"  size="60"><span>&nbsp;({__("platform_eg")})</span>
    </div>
</div>
...
<p>
    <a onclick="Tygh.$.ceAjax('request', '{"payment_notification.activate?payment=moneybookers&payment_id=`$payment_id`&email="|fn_url:'A':'rel' nofilter}' + Tygh.$('#pay_to_email_{$payment_id}').val() + '&cust_id=' + Tygh.$('#customer_id_{$payment_id}').val() + '&platform=' + Tygh.$('#platform_{$payment_id}').val() + '&merchant_firstname=' + Tygh.$('#merchant_firstname_{$payment_id}').val() + '&merchant_lastname=' + Tygh.$('#merchant_lastname_{$payment_id}').val(), {ldelim}method: 'GET'{rdelim}); return false;" href="{"payment_notification.activate?payment=moneybookers&payment_id=`$payment_id`&email=`$pay_to_email`&cust_id=`$processor_params.customer_id`&platform=`$processor_params.platform`"|fn_url}">{__("activate_moneybookers_merchant_tools")}</a>
    </p>
    <p>
    {__("text_moneybookers_activate_quick_checkout_short_explanation")}
    </p>
    <p>
    {__("text_moneybookers_activate_quick_checkout_short_explanation_2")}
    </p>
    <p>
    {__("text_mb_quick_checkout_payments")}
    </p>
<br>
<div class="control-group">
    <label class="control-label" for="secret_word_{$payment_id}">{__("secret_word")}:</label>
    <div class="controls">
        <input type="text" name="payment_data[processor_params][secret_word]" id="secret_word_{$payment_id}" value="{$processor_params.secret_word}"  size="60" onchange="Tygh.$('#validate_secret_word_{$payment_id}').prop( 'href', '{"payment_notification.validate_secret_word?payment=moneybookers&payment_id=`$payment_id`&email="|fn_url:'A':'rel' nofilter}' + Tygh.$('#pay_to_email_{$payment_id}').val() + '&cust_id=' + Tygh.$('#customer_id_{$payment_id}').val() + '&secret=' + Tygh.$(this).val()); return false;">&nbsp;<a href="{"payment_notification.validate_secret_word?payment=moneybookers&payment_id=`$payment_id`&email=`$pay_to_email`&cust_id=`$processor_params.customer_id`&secret=`$processor_params.secret_word`"|fn_url}" onclick="Tygh.$.ceAjax('request', Tygh.$(this).prop('href'), {ldelim}method: 'GET', callback: fn_get_validate_secret_word_{$payment_id}{rdelim}); return false;" id="validate_secret_word_{$payment_id}">{__("validate_secret_word")}</a>
        ...
        <p><small>{__("text_moneybookers_secred_word_notice")}</small></p>
    </div>
</div>

<div class="control-group">
    <label class="control-label" for="quick_checkout_{$payment_id}">{__("forbid_moneybookers_ewallet")}:</label>
    <div class="controls">
        <select name="payment_data[processor_params][quick_checkout]" id="quick_checkout_{$payment_id}">
            <option value="N" {if $processor_params.quick_checkout == 'N'}selected="selected"{/if}>{__("no")}</option>
            <option value="Y" {if $processor_params.quick_checkout == 'Y'}selected="selected"{/if}>{__("yes")}</option>
        </select>
        <p><small>{__("text_use_moneybookers_ewallet_description")}</small></p>
    </div>
</div>
...
<div class="control-group">
    ...
    <div class="controls">
        <input type="text" name="payment_data[processor_params][recipient_description]" id="recipient_description_{$payment_id}" value="{$processor_params.recipient_description}"  size="60">
    </div>
</div>
...
<div class="control-group">
    ...
    <div class="controls">
        ...
        <p><small>{__("text_moneybookers_currs_notice", ["[link]" => $cur_man])}</small></p>
    </div>
</div>
...
<div class="control-group">
    ...
    <div class="controls">
        ...
        <p><small>{__("multiple_selectbox_notice")}</small></p>
    </div>
</div>
...
<div class="control-group">
    ...
    <div class="controls">
        ...
        <p><small>{__("text_moneybookers_logo_notice")}</small></p>
    </div>
</div>

<div class="control-group">
    <label class="control-label" for="return_url_target_{$payment_id}">{__("return_url_target")}:</label>
    <div class="controls">
        <select name="payment_data[processor_params][return_url_target]" id="return_url_target_{$payment_id}">
            <option value="1" {if $processor_params.return_url_target == '1'}selected="selected"{/if}>_top</option>
            <option value="2" {if $processor_params.return_url_target == '2'}selected="selected"{/if}>_parent</option>
            <option value="3" {if $processor_params.return_url_target == '3'}selected="selected"{/if}>_self</option>
            <option value="4" {if $processor_params.return_url_target == '4'}selected="selected"{/if}>_blank</option>
        </select>
    </div>
</div>

<div class="control-group">
    <label class="control-label" for="cancel_url_target_{$payment_id}">{__("cancel_url_target")}:</label>
    <div class="controls">
        <select name="payment_data[processor_params][cancel_url_target]" id="cancel_url_target_{$payment_id}">
            <option value="1" {if $processor_params.cancel_url_target == '1'}selected="selected"{/if}>_top</option>
            <option value="2" {if $processor_params.cancel_url_target == '2'}selected="selected"{/if}>_parent</option>
            <option value="3" {if $processor_params.cancel_url_target == '3'}selected="selected"{/if}>_self</option>
            <option value="4" {if $processor_params.cancel_url_target == '4'}selected="selected"{/if}>_blank</option>
        </select>
    </div>
</div>

<p>{__("text_moneybookers_support")}</p>