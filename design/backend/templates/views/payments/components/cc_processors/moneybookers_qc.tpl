...
<div class="control-group">
    ...
    <div class="controls">
        <input type="text" name="payment_data[processor_params][pay_to_email]" id="pay_to_email_{$payment_id}" value="{$processor_params.pay_to_email}"  size="60" onchange="Tygh.$('#validate_email_{$payment_id}').prop('href', '{"payment_notification.validate_email?payment=moneybookers&payment_id=`$payment_id`&email="|fn_url:'A':'rel' nofilter}' +  Tygh.$(this).val()); Tygh.$('#validate_secret_word_{$payment_id}').prop('href', '{"payment_notification.validate_secret_word?payment=moneybookers&payment_id=`$payment_id`&email="|fn_url:'A':'rel' nofilter}' +  Tygh.$(this).val() + '&cust_id=' + Tygh.$('#customer_id_{$payment_id}').val() + '&secret=' + Tygh.$('#secret_word_{$payment_id}').val()); return false;">&nbsp;<a href="{"payment_notification.validate_email?payment=moneybookers&payment_id=`$payment_id`&email=`$processor_params.pay_to_email`"|fn_url}" onclick="Tygh.$.ceAjax('request', Tygh.$(this).prop('href'), {ldelim}method: 'GET', callback: fn_get_validate_email_{$payment_id}{rdelim}); return false;" id="validate_email_{$payment_id}">{__("validate_email")}</a>
    </div>
</div>
