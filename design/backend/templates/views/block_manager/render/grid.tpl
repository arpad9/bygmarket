        ...
        <div class="bm-full-menu grid-control-menu bm-control-menu {if $grid.width <= 2}hidden{/if}">
            {if $container.default == 1 || $container.position == 'CONTENT' && !$dynamic_object || $show_menu}
                ...
                <div class="grid-control-menu-actions">
                    ...
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown"><span class='icon-plus cm-tooltip' title="{__("add_grid_block")}"></span></a>
                    ...
                    <div class="cm-action bm-action-properties cm-tooltip exicon-cog action" title="{__("grid_options")}"></div>
                    <div class="cm-action bm-action-switch cm-tooltip exicon-off action" title="{__("enable_or_disable_grid")}"></div>
                    <div class="cm-action bm-action-delete cm-tooltip pull-right exicon-trash extra action" title="{__("delete_grid")}"></div>
                </div>
            {/if}
            <h4 class="grid-control-title {if $grid.width <= 2}hidden{/if}">{__("grid")}&nbsp;{$grid.width|default:"0"}</h4>
        </div>
        {if $container.default == 1 || $container.position == 'CONTENT' && !$dynamic_object || $show_menu}
