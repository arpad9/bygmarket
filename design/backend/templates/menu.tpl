    ...
    <div class="navbar navbar-inverse" id="header_navbar">
        ...
        {if "ULTIMATE"|fn_allowed_for}
            <div class="nav-ult">
                ...
                {if $runtime.company_data.storefront}
                    <a href="http://{$runtime.company_data.storefront}" target="_blank" class="brand" title="{__("view_storefront")}">
                ...
                {if $runtime.companies_available_count != 1}
                    ...
                    {capture name="extra_content"}
                        {if fn_check_view_permissions("companies.manage", "GET")}
                            <li class="divider"></li>
                            <li><a href="{"companies.manage"|fn_url}">{__("manage_stores")}...</a></li>
                        {/if}
                    {/capture}
                    {include file="common/ajax_select_object.tpl" data_url="companies.get_companies_list?show_all=Y&action=href" text=$name id="top_company_id" type="list" extra_content=$smarty.capture.extra_content}
                    </ul>
                {else}
                    <ul class="nav">
                        {if $auth.company_id}
                            ...
                            {if fn_check_view_permissions("companies.manage", "GET")}
                                <li class="dropdown vendor-submenu">
                                    <a class=" dropdown-toggle" data-toggle="dropdown">
                                        <span>{$name}</span><b class="caret"></b>
                                    </a>
        ...
        {if "MULTIVENDOR"|fn_allowed_for && !$runtime.simple_ultimate}
            <ul class="nav">
                ...
                <a href="{""|fn_url}" class="brand">{$name}</a>
                ...
                {else}
                    {if fn_check_view_permissions("companies.get_companies_list", "GET")}
                        {capture name="extra_content"}
                            <li class="divider"></li>
                            <li><a href="{"companies.manage"|fn_url}">{__("manage_vendors")}...</a></li>
                        {/capture}
        ...
        {/if}
                            ...
                            {foreach from=$m.items key=second_level_title item="second_level" name="sec_level_top"}
                                <li class="{if $second_level.subitems}dropdown-submenu{/if}{if $second_level_title == $navigation.subsection} active{/if} {if $second_level.is_promo}cm-promo-popup{/if} {$second_level.attrs.class}" {menu_attrs attrs=$second_level.attrs.main}>
                                    {if $second_level.type == "title"}
                                        <a {menu_attrs attrs=$second_level.attrs.href}>{$second_level.title|default:__($second_level_title)}</a>
                                    {elseif $second_level.type != "divider"}
                                        <a href="{$second_level.href|fn_url}" {menu_attrs attrs=$second_level.attrs.href}>{$second_level.title|default:__($second_level_title)}</a>
                                    {/if}
                                ...
                                {/if}
                            {/foreach}
                            {if $first_level_title == "settings" && (!$runtime.company_id || $runtime.simple_ultimate) && !"RESTRICTED_ADMIN"|defined}
                                <li class="divider"></li>
                                <li><a class="cm-dialog-opener cm-dialog-auto-size" data-ca-target-id="store_mode_dialog">{__("store_mode")}</a></li>
                            {/if}
                        </ul>
                    </li>
                {/foreach}
            ...
            {if $auth.user_id}
                ...
                <!--language-->
                {$c_url = $config.current_url|fn_query_remove:"is_ajax":"callback":"from_url":"full_render":"result_ids"}
                {if !"ULTIMATE:FREE"|fn_allowed_for}
                    {if $languages|sizeof > 1}
                        {include file="common/select_object.tpl" style="dropdown" link_tpl=$c_url|fn_link_attach:"sl=" items=$languages selected_id=$smarty.const.CART_LANGUAGE display_icons=true key_name="name" key_selected="lang_code" class="languages"}
                    {/if}
                {/if}
                <!--end language-->
                ...
                {include file="common/select_object.tpl" style="dropdown" link_tpl=$c_url|fn_link_attach:"currency=" items=$currencies selected_id=$secondary_currency display_icons=false key_name="description" key_selected="currency_code"}
                {/if}
                <!--end curriencies-->
