...
<table width="100%" class="table table-middle table-objects{if $table_striped} table-striped{/if}">
    <tbody>
{/if}
        <tr class="cm-row-status-{$status|lower} {$additional_class} cm-row-item" {if $row_id}id="{$row_id}"{/if}>
            {if $checkbox_name}
            <td>
                        ...
                        {if !$non_editable}
                            {if $href_delete && !$skip_delete}
                                ...
                                {else}
                                    ...
                                    {$href=$href_delete}
                                {/if}
                                <li>{btn type="text" text=__("delete") href=$href class="cm-confirm cm-tooltip cm-ajax cm-ajax-force cm-ajax-full-render `$class`" data=["data-ca-target-id" => $delete_target_id]}</li>
                            {/if}
                        {/if}
                    {/capture}
            ...
            {if !$nostatus}
                <td width="12%">
                    <div class="pull-right nowrap">
                        ...
                        {include file="common/select_popup.tpl" popup_additional_class="dropleft" id=$id status=$status hidden=$hidden object_id_name=$object_id_name table=$table hide_for_vendor=$hide_for_vendor display=$display non_editable=$non_editable}
                    </div>
                </td>
            {/if}
        <!--{$row_id}--></tr>
