...
{if $multiple}
    <tr {if !$clone}id="{$holder}_{$category_id}" {/if}class="cm-js-item {if $clone} cm-clone hidden{/if}">
        ...
        <td>{if !$show_only_name}<a href="{"categories.update?category_id=`$category_id`"|fn_url}">{$category}</a>{else}{$category} {include file="views/companies/components/company_name.tpl" object=$category_data}{/if}</td>
    ...
    {else}
        ...
        {if $first_item || !$category_id}
            <p class="cm-js-item cm-clone hidden">
                {if $hide_input != "Y"}
                <label class="radio" for="category_rb_{$default_category_id}"><input id="category_rb_{$default_category_id}" type="radio" name="{$radio_input_name}" value="{$default_category_id}">{/if}{$default_category}
                <a class="icon-remove-sign cm-tooltip hand hidden" onclick="Tygh.$.cePicker('delete_js_item', '{$holder}', '{$default_category_id}', 'c'); return false;" title="{__("remove")}"></a></label>
            </p>
                {/if}
        {if $category_id}
        <div class="cm-js-item {$extra_class}" id="{$holder}_{$category_id}" {$extra} />
            {if $hide_input != "Y"}
                <label class="radio" for="category_radio_button_{$category_id}"><input id="category_radio_button_{$category_id}"{if $main_category == $category_id}checked{/if} type="radio" name="{$radio_input_name}" value="{$category_id}" />
            {/if}
