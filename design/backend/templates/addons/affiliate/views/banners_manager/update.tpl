...
{if $runtime.mode == "add"}
    {include file="common/mainbox.tpl" title=__("new_banner") content=$smarty.capture.mainbox buttons=$smarty.capture.buttons select_languages=true}
{else}
    {include file="common/mainbox.tpl" title="{__("editing_banner")}: `$banner.title`" content=$smarty.capture.mainbox buttons=$smarty.capture.buttons adv_buttons=$smarty.capture.adv_buttons select_languages=true}
{/if}
