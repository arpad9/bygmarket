...
{capture name="add_new_picker"}
    ...
    <div class="tabs cm-j-tabs">
        <ul class="nav nav-tabs">
            <li id="tab_mailing_list_details_0" class="cm-js cm-active"><a>{__("general")}</a></li>
        </ul>
    </div>
    ...
    <fieldset>
        ...
        {if $mailing_lists}
        <div class="control-group">
            <label class="cm-required control-label">{__("mailing_lists")}</label>
            <div class="controls">
                {foreach from=$mailing_lists key=m_key item=m_value}
                    <label class="checkbox" for="subscriber_data_{$m_key}">
                    <input type="checkbox" id="subscriber_data_{$m_key}" name="subscriber_data[list_ids][]" value="{$m_key}">{$m_value}
                    </label>
                {/foreach}
            </div>
        </div>
        ...
        <div class="control-group">
            <label for="elm_conf_0" class="control-label">{__("confirmed")}</label>
            <div class="controls">
                <input type="hidden" name="subscriber_data[confirmed]" value="0" />
                <input id="elm_conf_0" type="checkbox" name="subscriber_data[confirmed]" value="1" />
            </div>
        </div>

        <div class="control-group">
            <label class="control-label" for="elm_notify_0">{__("notify_user")}</label>
            <div class="controls">
                <input type="hidden" name="subscriber_data[notify_user]" value="0" />
                <input id="elm_notify_0" type="checkbox" name="subscriber_data[notify_user]" value="1"/>
            </div>
        </div>
...
{capture name="adv_buttons"}
    {capture name="tools_list"}
        ...
        <li>{include file="pickers/users/picker.tpl" data_id="subscr_user" picker_for="subscribers" extra_var="subscribers.add_users?list_id=`$smarty.request.list_id`" but_text=__("add_subscribers_from_users") view_mode="button" no_container=true}</li>
    {/capture}
