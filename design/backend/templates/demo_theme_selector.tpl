...
<ul class="demo-site-panel cm-demo-site-panel unstyled clearfix">
    ...
    {if "ULTIMATE"|fn_allowed_for}    
        <li class="dp-area">
            <select name="area" onchange="Tygh.$.redirect(this.value);">
                <option value="{$config.admin_index}" {if $area == "A"}selected="selected"{$selected = true}{/if}>{__("admin_panel")}</option>

                {foreach from=$demo_theme.storefronts item=company}
                    <option value="http://{$company.storefront}" {if !$selected && $runtime.company_id == $company.company_id}selected="selected"{/if}>{$company.company}</option>
                {/foreach}
            </select>
        </li>
