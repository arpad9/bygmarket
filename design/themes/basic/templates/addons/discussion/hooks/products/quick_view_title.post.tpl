{if $product.discussion_type && $product.discussion_type != 'D'}
    <div class="rating-wrapper clearfix" id="average_rating_product_{$obj_prefix}{$obj_id}">
        ...
        <a  href="{"products.view?product_id=`$product.product_id`&selected_section=discussion#discussion"|fn_url}">{$product.discussion.posts|count} {__("reviews")}</a>
        {/if}
        <a class="cm-dialog-opener cm-dialog-auto-size" data-ca-target-id="new_post_dialog_{$obj_prefix}{$obj_id}">{__("write_review")}</a>
