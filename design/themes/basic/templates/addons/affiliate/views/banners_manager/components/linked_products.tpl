...
{foreach from=$linked_products item=product}
    <tr {cycle values=",class=\"table-row\""}>
        ...
        <td>
            {include file="common/popupbox.tpl" id="product_`$product.product_id`" link_text=$product.product text=__("product") href="banner_products.view?product_id=`$product.product_id`"}</td>
        <td>
            {$product.url}</td>
    </tr>
{foreachelse}
