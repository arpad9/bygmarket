...
GEvent.addListener(marker, "click", function() {literal}{{/literal}
    var html = '<div style="padding-right: 10px"><strong>{$loc.name|escape:javascript nofilter}</strong><p>{$loc.city|escape:javascript nofilter}, {$loc.country_title|escape:javascript nofilter}</p><\/div>';
...
<p>{include file="buttons/button.tpl" but_role="text" but_onclick="fn_move_map('`$loc.store_location_id`'); fn_show_marker_`$loc.store_location_id`();" but_text=__("view_on_map")}</p>
</div>
{if $num == 0}
