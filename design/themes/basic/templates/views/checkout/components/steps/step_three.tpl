<div class="step-container{if $edit}-active{/if}" id="step_three">
    {if $settings.General.checkout_style != "multi_page"}
        <h2 class="step-title{if $edit}-active{/if}{if $complete && !$edit}-complete{/if} clearfix">
            ...
            <a class="title{if $complete && !$edit} cm-ajax{/if}" {if $complete && !$edit}href="{"checkout.checkout?edit_step=step_three&from_step=`$edit_step`&is_ajax=1"|fn_url}" data-ca-target-id="checkout_*"{/if}>{__("shipping_options")}</a>
            {/hook}
        </h2>
    {/if}
