    ...
    {/if}
        ...
        {hook name="checkout:product_info"}
            ...
            {elseif $product.discount|floatval || ($product.taxes && $settings.General.tax_calculation != "subtotal")}
                ...
                {/if}
                <p><a data-ca-target-id="discount_{$key}" class="cm-dialog-opener cm-dialog-auto-size">{$price_info_title}</a></p>
        ...
        <div class="clear"></div>
        <a id="sw_options_{$key}" class="cm-combo-on cm-combination"><i>{__("text_click_here")}</i></a>
