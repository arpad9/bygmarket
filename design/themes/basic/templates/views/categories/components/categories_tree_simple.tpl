...
<tr>
    ...
    <th style="width: 97%">
        ...
        <div class="float-left">
            <img src="{$images_dir}/icons/plus_minus.gif" width="13" height="12" id="on_cat" alt="{__("expand_collapse_list")}" title="{__("expand_collapse_list")}" class="hand cm-combinations {if $expand_all}hidden{/if}"  />
            <img src="{$images_dir}/icons/minus_plus.gif" width="13" height="12" id="off_cat" alt="{__("expand_collapse_list")}" title="{__("expand_collapse_list")}" class="hand cm-combinations {if !$expand_all}hidden{/if}" />
        </div>
        {/if}
        &nbsp;{__("categories")}
    </th>
        ...
        <tr>
            <td class="nowrap" style="padding-left: {$_shift}px;">
                {if $cur_cat.has_children || $cur_cat.subcategories}
                    {if $show_all}
                    <img src="{$images_dir}/icons/plus.gif" width="14" height="9" alt="{__("expand_sublist_of_items")}" title="{__("expand_sublist_of_items")}" id="on_{$comb_id}" class="hand cm-combination {if isset($path.$cat_id) || $expand_all}hidden{/if}" />
                    {else}
                    <img src="{$images_dir}/icons/plus.gif" width="14" height="9" alt="{__("expand_sublist_of_items")}" title="{__("expand_sublist_of_items")}" id="on_{$comb_id}" class="hand cm-combination {if (isset($path.$cat_id))}hidden{/if}" onclick="if (!Tygh.$('#{$title_id}').children().get(0)) Tygh.$.ceAjax('request', '{"categories.picker?category_id=`$cur_cat.category_id`&display=`$display`"|fn_url}', {$ldelim}result_ids: '{$comb_id}'{$rdelim})" />
                    {/if}
                    <img src="{$images_dir}/icons/minus.gif" width="14" height="9" alt="{__("collapse_sublist_of_items")}" title="{__("collapse_sublist_of_items")}" id="off_{$comb_id}" class="hand cm-combination {if !isset($path.$cat_id) && (!$expand_all || !$show_all)}hidden{/if}" />
                {else}
