...
<tr>
    <td><div style="padding-left: {$shift}px;">
        <i class="icon-file"></i>
        {if $product_file.ekey && ($product_file.activation_type !== "M" || $product_file.active == "Y") && $product_file.edp_info && (!$product_file.max_downloads || $product_file.downloads < $product_file.max_downloads)}<a href="{"orders.get_file?ekey=`$product_file.ekey`&product_id=`$product.product_id`&file_id=`$product_file.file_id`"|fn_url}" class="download-link">{$product_file.file_name}</a><i class="icon-download"></i>{else}{$product_file.file_name}{/if}
