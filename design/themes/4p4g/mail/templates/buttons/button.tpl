...
{elseif !$but_role} {* DEFAULT INPUT BUTTON *}
    <input {if $but_id}id="{$but_id}"{/if} class="default-button{if $but_meta} {$but_meta}{/if}" type="submit" onclick="{$but_onclick};{if !$allow_href} return false;{/if}" value="{$but_text}" {if $but_external_click_id} data-ca-external-click-id="{$but_external_click_id}"{/if}{if $but_target_form} data-ca-target-form="{$but_target_form}"{/if}{if $but_target_id} data-ca-target-id="{$but_target_id}"{/if} />
{/if}
