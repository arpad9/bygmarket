...
{if $view_only != "Y"}    
        <div class="orders-print">
            ...
            <span><i class="icon-print"></i>{include file="buttons/button_popup.tpl" but_role="text" but_text=$print_order but_href="orders.print_invoice?order_id=`$order_info.order_id`" width="900" height="600"}</span>
            <span><i class="icon-doc-text"></i>{include file="buttons/button.tpl" but_role="text" but_meta="pdf" but_text=$print_pdf_order but_href="orders.print_invoice?order_id=`$order_info.order_id`&format=pdf"}</span>        
            {/hook}
            ...
            {else}
                {foreach from=$order_info.shipping item="shipping" name="f_shipp"}
                    {if $shipments[$shipping.group_key].carrier && $shipments[$shipping.group_key].tracking_number}
                        ...
                        {$shipping.shipping}&nbsp;({__("tracking_num")}<a {if $smarty.capture.carrier_url|strpos:"://"}target="_blank"{/if} href="{$smarty.capture.carrier_url}">{$shipments[$shipping.group_key].tracking_number}</a>)
                    {else}
                        {$shipping.shipping}
                    {/if}
    ...
    <div id="content_shipment_info" class="orders-shipment {if $selected_section != "shipment_info"}hidden{/if}">
        {foreach from=$shipments key="id" item="shipment"}
            ...
            {if $shipment.carrier && $shipment.tracking_number}
                ...
                {$shipment.shipping}&nbsp;({__("tracking_num")}<a {if $smarty.capture.carrier_url|strpos:"://"}target="_blank"{/if} href="{$smarty.capture.carrier_url}">{$shipment.tracking_number}</a>)
            {else}
                {$shipment.shipping}
            {/if}
            </div>
