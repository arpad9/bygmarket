<div class="step-container{if $edit}-active{/if}" id="step_one">
    {if $settings.General.checkout_style != "multi_page"}
        <h2 class="step-title{if $edit}-active{/if}{if $complete && !$edit}-complete{/if} clearfix">
            ...
            <a class="title{if $contact_info_population && !$edit} cm-ajax{/if}" {if $contact_info_population && !$edit}href="{"checkout.checkout?edit_step=step_one&from_step=`$edit_step`"|fn_url}" data-ca-target-id="checkout_*"{/if}>{$title nofilter}</a>
            {/hook}
        </h2>
    {/if}
