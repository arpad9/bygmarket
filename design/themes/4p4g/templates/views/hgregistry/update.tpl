...
$(document).ready(function(){
	...
	$('.icon-cancel-circle').on('click',function(){
		var r = confirm('Are you sure want to delete this product from your list');
		if (r==true)
		{
		  return true;
		}
...
<div class="weddingInfoContainer">
	...
	<div class="registryInfoTitle">{$event_data.yourfname} {$event_data.yourlname}{if !empty($event_data.theirfname) && !empty($event_data.theirlname)} and {$event_data.theirfname} {$event_data.theirlname}{/if}'s Holiday or Gift Registry</div>
	<div class="registryInfoWelcome"><span><b>Hello, {$user_info.firstname} {$user_info.lastname}</b> Welcome to your Holiday or Gift Registry.</span> ({include file="buttons/button.tpl" but_text="Manage Your Registry" but_href="hgregistry.addreg&hgr_id=$registry_type_id" but_role="text" but_meta="modified"})</div>
	<div class="registryInfoPrivacy">
		...
		{/if}
		({include file="buttons/button.tpl" but_text="Change Settings" but_href="hgregistry.addreg&hgr_id=$registry_type_id" but_role="text" but_meta="modified"})
	</div>
...
<form action="{""|fn_url}" method="post" name="holiday_registry_products_form" >
{*
<div class="buttons-container">
    &nbsp;{include file="buttons/button.tpl" but_text=__("update") but_name="dispatch[hgregistry.update]"}
    <div class="float-left">
        {include file="pickers/products/picker.tpl" data_id="hgr_products" but_text=__("add_product") extra_var="hgregistry.add_products?registry_type_id=`$registry_type_id`" no_container=true}
    </div>
</div>
*}
        ...
        <tbody>
            ...
            <tr>
                <td>
                    ...
                    <a href="{"hgregistry.delete_product?item_id=`$key`&registry_type_id=`$registry_type_id`"|fn_url}" class="icon-delete-big" title="{__("remove")}"><i class="icon-cancel-circle"></i></a>
...
{include file="pickers/products/picker.tpl" data_id="hgr_products" but_text=__("add_product") extra_var="hgregistry.add_products?registry_type_id=`$registry_type_id`"}
{/if}
</div>
