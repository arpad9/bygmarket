...
{script src="js/lib/bootstrap/js/bootstrap.js"}
{script src="js/tygh/4p4g.js"}
...
(function(_, $) {
    ...
    {if $smarty.request.init_context}
        var url = $.ceHistory('parseHash', window.location.hash) || fn_url('index.index');

        // in embedded mode if other jQuery library exists on the page, restore first loaded version
        if (_.embedded && _.otherjQ) {
            $.noConflict(true);
        }

        $.ceAjax('request', url, {
            result_ids: _.init_container,
            data: {
            ...
            pre_processing: function() {
                $('#tygh_ldr').remove();
                $('#' + _.init_container).show();
            },
            on_ajax_done: function() {
                $.runCart('C', $('#' + _.init_container));
            }
        });
