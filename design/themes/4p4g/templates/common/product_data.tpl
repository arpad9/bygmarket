...
{if !$hide_form}
<form action="{""|fn_url}" method="post" name="product_form_{$obj_prefix}{$obj_id}" enctype="multipart/form-data" class="cm-disable-empty-files {if $is_ajax} cm-ajax cm-ajax-full-render{/if} {if $form_meta}{$form_meta}{/if}">
    ...
    {if $show_sku}
        <p class="sku{if !$product.product_code} hidden{/if}">
            <span class="cm-reload-{$obj_prefix}{$obj_id}" id="sku_update_{$obj_prefix}{$obj_id}">
                ...
                <span id="sku_{$obj_prefix}{$obj_id}">{__("sku")}: <span id="product_code_{$obj_prefix}{$obj_id}">{$product.product_code}</span></span>
...
{capture name="discount_label_`$obj_prefix``$obj_id`"}
    {if $show_discount_label && ($product.discount_prc || $product.list_discount_prc) && $show_price_values}
        <ul class="discount-label cm-reload-{$obj_prefix}{$obj_id}" id="discount_label_update_{$obj_prefix}{$obj_id}">
            <li id="line_prc_discount_value_{$obj_prefix}{$obj_id}"><span id="prc_discount_value_label_{$obj_prefix}{$obj_id}">-{if $product.discount}{$product.discount_prc}{else}{$product.list_discount_prc}{/if}%</span></li>
...
{capture name="qty_`$obj_id`"}
    ...
    {if $show_qty && !$hide_qty_products.$obj_id}
