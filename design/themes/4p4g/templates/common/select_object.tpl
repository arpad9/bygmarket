...
{if $style == "graphic"}
    ...
    <a class="select-link cm-combo-on cm-combination" id="sw_select_{$selected_id}_wrap_{$suffix}"><span>{$items.$selected_id.$key_name}{if $items.$selected_id.symbol} ({$items.$selected_id.symbol nofilter}){/if}</span><i class="icon-down-micro"></i></a>
