...
{foreach from=$items item="filter" name="filters"}
    {if $filter.slider || $filter.selected_ranges || $filter.ranges}
        ...
        <div id="sw_content_{$filter_uid}" class="filter-wrap cm-combination-filter_{$filter_uid} cm-combo-{if $collapse}on{else}off{/if}">
            <i class="icon-down-dir"></i><i class="icon-right-dir"></i>
            <span class="filter-title cm-save-state {if $filter.display == "Y"}cm-ss-reverse{/if}">{$filter.filter}</span>
...
<div class="filters-tools clearfix">
    ...
    <a href="{if $smarty.request.category_id}{assign var="use_ajax" value=true}{"categories.view?category_id=`$smarty.request.category_id`"|fn_url}{else}{assign var="use_ajax" value=false}{""|fn_url}{/if}" rel="nofollow" class="reset-filters{if $allow_ajax && $use_ajax} cm-ajax cm-ajax-full-render cm-history" data-ca-target-id="{$ajax_div_ids}{/if}"><i class="icon-cw"></i> {__("reset")}</a>
    {/if}
</div>
