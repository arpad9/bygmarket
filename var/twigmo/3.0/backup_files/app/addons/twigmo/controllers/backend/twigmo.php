...
if ($mode == 'post') {
    if ($_REQUEST['action'] == 'get') {
        ...
        if ($object == 'statuses') {
            ...
            $lessThanPrevMonth = $today['mday'] < $days_in_prev_month;
            $prev_mday = $lessThanPrevMonth ? $today['mday'] : $days_in_prev_month;
            $prev_periods =  array (
                'day' => array (
                    'start' => mktime(
                        0,
            ...
            foreach ($stats_periods as $title => $time_from) {
                ...
                $totals = db_get_row(
                    "SELECT SUM(IF(status = 'C' OR status = 'P', total, 0)) as total_paid,
                        SUM(total) as total,
                ...
                $order_amount_change = $order_amount -
                (empty($prev_totals['order_amount']) ? 0 : $prev_totals['order_amount']);
                $total_paid_change = $period_total_paid -
                (empty($prev_totals['total_paid']) ? 0 : $prev_totals['total_paid']);
