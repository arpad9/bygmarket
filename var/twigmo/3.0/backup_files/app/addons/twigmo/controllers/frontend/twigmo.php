...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $mode == 'post') {
    ...
    if ($meta['action'] == 'login') {
        ...
        $cart = & $_SESSION['cart'];
        $ids = fn_twg_api_add_product_to_cart(array($data), $cart);
    ...
    //        fn_api_set_cart_user_data($data['user'], $response, $lang_code);
            ...
            if (fn_twg_payment_requested($_REQUEST)) {
                ...
                if (!empty($_REQUEST['payment_info'])) {
                    ...
                    if (!empty($cart['extra_payment_info']['card_number'])) {
                        $cart['extra_payment_info']['secure_card_number'] =
                        preg_replace(
                            '/^(.+?)([0-9]{4})$/i',
                            '***-$2',
                            $cart['extra_payment_info']['card_number']
                        );
                    }
                } else {
        ...
        if ($meta['object'] == 'page') {
            ...
            if (!empty($_REQUEST['shipping_ids'])) {
                // Map shipping id to key in shippings list
                $params = array(
                    'cart' => & $_SESSION['cart'],
                    'auth' => & $_SESSION['auth'],
                    'companies_rates' => isset($_SESSION['companies_rates']) ? $_SESSION['companies_rates'] : false
                );
                list($shippings, $companies_rates, $_SESSION['shipping_rates']) = fn_twg_api_get_shippings($params);
                foreach($shippings as $key=>$shipping) {
                    foreach ($_REQUEST['shipping_ids'] as $group_id => $shipping_id) {
                        if ($shipping_id == $shipping['shipping_id']){
                            $_REQUEST['shipping_ids'][$group_id] = $key;
                        }
                    }
                }
            ...
            $data = fn_twg_get_api_data($response, $format, false);
