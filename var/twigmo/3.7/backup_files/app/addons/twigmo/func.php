...
// TODO: delete this method in twigmo 3.1
function fn_twigmo_exclude_unused_settings($settings)
{
    $old_fields = array(
        'access_id',
        'secret_access_key',
    ...
    $old_use_mfe = $settings['use_mobile_frontend'];
    $settings['use_for_phones'] = $old_use_mfe == 'both_tablet_and_phone' || $old_use_mfe == 'phone' ? 'Y' : 'N';
    $settings['use_for_tablets'] = $old_use_mfe == 'both_tablet_and_phone' || $old_use_mfe == 'tablet' ? 'Y' : 'N';
    unset($settings['use_mobile_frontend']);

    if (isset($settings['logoURL'])) {
        $settings['logo_url'] = $settings['logoURL'];
        unset($settings['logoURL']);
    }
    if (isset($settings['faviconURL'])) {
        $settings['favicon_url'] = $settings['faviconURL'];
        unset($settings['faviconURL']);
    }
    return $settings;
}
