...
class TwigmoUpgrade
{
    ...
    final public static function restoreSettingsAndCSS($upgrade_dirs)
    {
        ...
        // TODO: [antev] Implement it in 3.1 for 3.x-3.x upgrade
//    $all_settings = unserialize(fn_get_contents($upgrade_dirs['backup_settings'] . 'settings_all.bak'));
//    $all_settings = fn_twigmo_exclude_unused_settings($all_settings);
//    TwigmoSettings::set($all_settings);
//    if (TwigmoConnector::getAccessID()) {
//        $connector = new TwigmoConnector();
//        $connector->updateConnections(true);
//    }
        // Restore companies settings
        foreach ($upgrade_dirs['backup_company_settings'] as $company_id => $dir) {
            $company_settings = unserialize(fn_get_contents($dir . 'settings.bak'));
            $company_settings['version'] = TWIGMO_VERSION;
            $company_settings = fn_twigmo_exclude_unused_settings($company_settings);
            TwigmoSettings::set(array('customer_connections' => array($company_id => $company_settings)));
        }
    }
