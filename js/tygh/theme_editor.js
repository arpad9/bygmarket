    ...
    function initGoogleFonts()
    {
        ...
        for (var k in _.google_fonts) {
            ...
            for(i = 0; i < _.google_fonts[k].length; i++) {
                lis[k] += '<li class="cm-te-google-font te-google-font-' + k + '" data-ca-select-box-value="'+ _.google_fonts[k][i]['name'] +'">' + '<span style="background-position: 0 -' + (i * step) +'px">' + _.google_fonts[k][i]['name'] + '</span>' + '</li>';
            }
        }
        ...
        container.each(function() {
            ...
            if (active.length) {
                ...
                self.find('span:first').html(text);
            }
        });
    }
    ...
    function updateLogos()
    {
        $('div.cm-te-logo').each(function() {
            ...
            if (self.data('caImageArea') && self.data('caImageArea') == 'theme') {
                $('img.logo,.ty-logo-container__image', 'div.logo-container,div.ty-logo-container').prop('src', getUrlFromCss(self.css('background-image'))).css({
                    width: 'auto',
                    height: 'auto'
                });
            }
        ...
        $.ceAjax('request', fn_url('theme_editor.view'), {
            ...
            callback: function() {
                ...
                $('#theme_editor').on('click', '.cm-te-google-font', function() {
                    ...
                    $('link:last').after('<link href="//fonts.googleapis.com/css?family=' + self.data('caSelectBoxValue') + ('weight' in font_data ? ':' + font_data['weight'] : '') + '" rel="stylesheet" type="text/css">');
